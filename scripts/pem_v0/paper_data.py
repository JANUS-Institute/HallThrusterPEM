...
VAR_DICT = load_variables(['V_vac', 'P*', 'PT', 'u_n', 'f_n', 'vAN1', 'vAN2', 'vAN3', 'vAN4', 'delta_z', 'z0', 'c0', 'c1', 'c2', 'c3', 'c4', 'c5', 'sigma_cex', 'r_m'], model_config_dir() / 'variables_v3.json')
print(THETA_VARS)
...
def run_models(Ns=1000):
    ...
    with h5py.File(f'monte-carlo.h5', 'a') as fd:
        ...
        xs = SURR.sample_inputs(sample_shape, use_pdf=False, nominal=nominal, constants=CONSTANTS)
        print(xs)
...
def spt100_monte_carlo(Ns=1000, plot=True):
    ...
    with h5py.File(file, 'r') as fd, plt.style.context("uqtils.default"):
        ...
        if plot:
            ...
            ax[1].set_ylim([-4, 22])
            fig.savefig('mc-uion-post.pdf', bbox_inches='tight', format='pdf')
            # plt.show()
