...
def test_cathode_coupling(plots=False):
    """Test prediction of cathode coupling voltage."""
    VCC_LB = 0
    VCC_UB = 100

    # Test scalar usage
    inputs = {'P_b': 10e-6, 'V_d': 300, 'T_e': 3, 'V_vac': 30, 'Pstar': 20e-6, 'P_T': 50e-6}
    outputs = cathode_coupling(inputs)

    # Test vectorized usage
    N = 100
    inputs_rand = {'P_b': 10 ** (np.random.rand(N) * 4 - 8), 'V_d': np.random.rand(N) * 200 + 200,
                   'T_e': np.random.rand(N) * 4 + 1, 'V_vac': np.random.rand(N) * 60,
                   'Pstar': np.random.rand(N) * 90e-6 + 10e-6, 'P_T': np.random.rand(N) * 90e-6 + 10e-6}
    outputs_rand = cathode_coupling(inputs_rand)

    assert np.all(outputs_rand['V_cc'] >= VCC_LB) and np.all(outputs_rand['V_cc'] <= VCC_UB)

    # Test single 1d sweep
    inputs_sweep = {'P_b': 10 ** (np.linspace(-6, -4, N)), 'V_d': 300, 'T_e': 1.33, 'V_vac': 31.6,
                    'Pstar': 24.6e-6, 'P_T': 10.2e-6}
    outputs_sweep = cathode_coupling(inputs_sweep)
