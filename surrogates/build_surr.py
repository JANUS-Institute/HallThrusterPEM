...
def train():
    with MPICommExecutor(MPI.COMM_WORLD, root=0) as executor:
        if executor is not None:
            ...
            end_time_hr = 6
            with open(Path('../models/data') / 'thruster_svd.pkl', 'rb') as fd:
                d = pickle.load(fd)
                r1 = d['vtr'].shape[0]
            with open('test_set.pkl', 'rb') as fd:
                test_set = pickle.load(fd)  # Dict('xt': array(Nt, xdim), 'yt': array(Nt, ydim))
            qoi_ind = [1, 2, 8, 9]  # just I_B0, thrust, and first 2 u_ion svd coeffs
            # qoi_ind = [0, 1, 2, 7, 8, int(7 + r1), int(7 + r1 + 1), int(7 + r1 + 2)]
            sys.build_system(qoi_ind=qoi_ind, N_refine=100, max_iter=100, max_tol=1e-4, max_runtime=end_time_hr,
