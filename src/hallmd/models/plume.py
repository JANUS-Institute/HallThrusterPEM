...
from hallmd.utils import ModelRunException, TORR_2_PA
...
def current_density(inputs: Dataset, j_ion_coords: np.ndarray = None):
    ...
    try:
        ...
        num_int = np.flip(j_ion, axis=-1) * np.cos(alpha_rad) * np.sin(alpha_rad)
        den_int = np.flip(j_ion, axis=-1) * np.cos(alpha_rad)
        ...
        raise ModelRunException(f"Exception in plume model: {e}") from e
